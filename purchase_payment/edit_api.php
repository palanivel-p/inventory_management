...
//    $sqlValidateCookie="SELECT * FROM `user_details` WHERE panel_api_key='$api_key'";
    ...
    if(mysqli_num_rows($resValidateCookie) > 0) {
        ...
        if (mysqli_num_rows($resValidate) > 0)  {
            ...
            $sqlUpdateBank = "UPDATE `bank_details` SET `ref_id`='$repayment_id',`bank_name`='$bank_name',`payment_date`='$repayment_date',`pay_from`='Purchase_payment',`customer_name`='$Supplier_name',`pay_mode`='$repayment_mode',`amount`='$pay_made',`ref_no`='$ref_no',`type` = 'Debit' WHERE `ref_id`='$repayment_id'";
